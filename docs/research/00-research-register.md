...
| R-030 | Opt-in Obsidian theme following, Style Settings surface, and Fable/Astra model registration | **Verified for source findings and CLI model probes; `gpt-6-astra` Plan path partially verified pending a Codex CLI upgrade** | **Mandatory for chat appearance and chat model catalog work** | [R-030 report](R-030-opt-in-theme-following-and-style-settings.md) |
